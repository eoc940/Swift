...
struct GetPersonDataService {
    ...
    func getPersonInfo(completion: @escaping (NetworkResult<Any>) -> Void) {
        ...
        let URL = "http://10.202.41.179:8080/echo"
        let header : HTTPHeaders = ["Content-Type" : "application/json"]
        
        /*
         ** "나 요렇게 통신 요청 보낼거야!!" 라고 적어둔 요청서라고 보면 된다
         URL 주소를 가지고, GET 방식을 통해, JSONEncoding 인코딩 방식으로, 헤더 정보와 함께
         Request를 보내기 위한 정보를 묶어서 dataRequest에 저장해둔다
         */
    ...
    // 아까 받은 statusCode를 바탕으로 어떻게 결과값을 처리할 것인지 정의한다
    ...
    // 200대로 status가 떨어졌을 때 데이터를 가공하기 위한 함수
